...
void main() {
    ...
    // Normal Mapping
    vec3 fragNorm = normalize(v_norm);
    vec3 fragTgt = normalize(v_tgt);
    vec3 fragBinorm = normalize(v_binorm);

    mat3 tbn;
    tbn[0][0] = fragTgt.x;
    tbn[1][0] = fragTgt.y;
    tbn[2][0] = fragTgt.z;
    tbn[0][1] = fragBinorm.x;
    tbn[1][1] = fragBinorm.y;
    tbn[2][1] = fragBinorm.z;
    tbn[0][2] = fragNorm.x;
    tbn[1][2] = fragNorm.y;
    tbn[2][2] = fragNorm.z;

    vec4 texNormalMap = texture2D(u_normalMap, v_texCoord);
    vec3 normal = normalize(texNormalMap * 2.0 - 1.0).xyz;
